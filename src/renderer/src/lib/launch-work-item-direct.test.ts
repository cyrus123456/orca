...
describe('launchWorkItemDirect', () => {
  ...
  it('plans direct SSH workspace agent startup for the remote host platform', async () => {
    ...
    expect(activationOptions.startup.command).toContain(`command test -n "$fish_pid" && set --erase -g ORCA_PI_PREFILL; command test -z "$fish_pid" && unset ORCA_PI_PREFILL; true`)
